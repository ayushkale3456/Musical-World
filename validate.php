        ...
        if (preg_match("/^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,3})$/", $email_address)) {
            if (preg_match("/^(\+?\d{1,4})?[-.\s]?(\()?(\d{1,3})(?(2)\))[-.\s]?\d{1,4}[-.\s]?\d{1,9}$/", $mobile_number)) {
                ...
                if (mysqli_num_rows($result_email) > 0) {
                    ...
                    $sql = "INSERT INTO user (`username`,`password`,`mobile_number`,`email_address`,`activation_code`) VALUES('$username','$hash_password','$mobile_number','$email_address','0')";
                    
                    // $sql = "SELECT * FROM user WHERE email_address = '$email_address' ";
                    $result = mysqli_query($conn, $sql);
                    // $row = mysqli_fetch_array($result);
                    // $username = $row['username'];
                    mysqli_query($conn, "UPDATE user SET confirm_status='1' WHERE email_address='" . $email_address . "' AND activation_code='0' AND confirm_status='0'") or die(mysqli_error($conn));
                    echo '<script type="text/javascript">
                setTimeout(function () { 
                    swal({
                        title:"Success",
                        type:"success",
                        text:"Thank you  your account has been activated successfully",
                },function(){
    ...
    if (!$result) {
        ...
        if ($count == 1) {
            if ($row['confirm_status'] == 0) {
                ...
                if ($row['email_address'] == 'ayushkale35@gmail.com' && $row['password'] == 'c12b240b5710c6c9ee00ef4529803aac') {
