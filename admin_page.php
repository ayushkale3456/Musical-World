<?php
	...
	if(isset($_POST['upload_tamil'])){
		...
		if(isset($_FILES['audio_file'])){
			...
			move_uploaded_file($file_tmp,"songs/tamil_songs/".$file_name);
         }
         ...
         if(isset($_FILES['song_image'])){
			...
			move_uploaded_file($file_tmp,"songs/tamil_songs/img/".$file_name);
		 }

		$sql = "INSERT INTO tamil_albums(`song_name`,`song_format`,`singer_name`,`movie_name`,`song_image`,`audio_file`) VALUES('$song_name','$song_format','$singer_name','$movie_name','$song_image','$audio_file')";
		$result = mysqli_query($conn,$sql);
		if($result){
    ...
    $sql = "SELECT * FROM tamil_albums ORDER BY song_id ASC";
	$result = mysqli_query($conn,$sql);
	...
	while($rows = mysqli_fetch_array($result)){
		...
		if(isset($_POST[$song_id])){
			...
			$sql = "SELECT * FROM tamil_albums WHERE song_id = '$song_id' ";
			...
			$sql = "DELETE FROM tamil_albums WHERE song_name = '$song_name' ";
			$results = mysqli_query($conn,$sql);

			if($results){
				echo '<script type="text/javascript">';
                echo 'setTimeout(function () { sweetAlert("Deleted"," <b>Song '.$song_name.' is deleted from tamil albums</b>","success");';
				echo '}, 500);</script>';
			}else{
				echo '<script type="text/javascript">';
	...
	<div class="form-row">
		...
		<div class="col">
			<div class="md-form mb-4">
				...
				<label data-error="wrong" data-success="right" for="form4">Album Name</label>
			</div>
		</div>
		
		<div class="col">
			<div class="text-center">
				<button type="submit" name="upload_tamil" class="btn btn-default btn-rounded mb-4">Upload <i class="fa fa-upload"></i></button>
			</div>
		</div>
	</div>	
</form>
...
<?php 
    ...
    $sql = "SELECT * FROM tamil_albums order by song_id";
    $result = mysqli_query($conn,$sql);

    echo "
        <div class='card'>
            <h3 class='card-header text-center font-weight-bold text-uppercase py-4'>tamil Songs</h3>
            <div class='card-body'>
                <div id='table' class='table-editable'>
                    <table class='table table-bordered table-responsive-md table-striped text-center'>
                        <tr>
	...
	<div class="form-row">
		...
		<div class="col">
			<div class="md-form mb-4">
				...
				<label data-error="wrong" data-success="right" for="form8">Album Name</label>
			</div>
		</div>
	...
	<div class="form-row">
		...
		<div class="col">
			<div class="md-form mb-4">
				...
				<label data-error="wrong" data-success="right" for="form12">Album Name</label>
			</div>
		</div>
